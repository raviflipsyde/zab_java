...
public class NettyClient {
	...
	public NettyClient(final NodeServer server){
...
//			SocketAddress remoteAddress = new InetSocketAddress("localhost", 9000);
//			
//			ChannelFuture f = b.connect(remoteAddress).sync();
//			Channel ch = f.channel();
//			ChannelFuture lastWriteFuture = ch.writeAndFlush(Unpooled.copiedBuffer("Hello World...123\r\n", StandardCharsets.UTF_8));
//			
//			remoteAddress = new InetSocketAddress("localhost", 9001);
//			f = b.connect(remoteAddress).sync();
//			ch = f.channel();
//			lastWriteFuture = ch.writeAndFlush(Unpooled.copiedBuffer("Hello World...123\r\n", StandardCharsets.UTF_8));
		} catch (Exception e) {
	...
	public  void sendMessage(String ip, int port, String msg){
		try {
			...
			ChannelFuture f = b.connect(remoteAddress).sync();
			Channel ch = f.channel();
			ChannelFuture lastWriteFuture = ch.writeAndFlush(Unpooled.copiedBuffer(msg+"\r\n", StandardCharsets.UTF_8));
